...
func ParserMalYaml(url, path string, clientConfig MalHTTPConfig) (MalsYaml, error) {
	var malData MalsYaml
	resp, body, err := httpRequest(clientConfig, url, http.Header{})
	if err != nil {
		return malData, err
	}
	...
	filePath := filepath.Join(path, MalIndexFileName)
